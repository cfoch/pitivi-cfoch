...
from pitivi.dialogs.imagesequences import ImageSequenceDialog
from pitivi.utils.ui import beautify_length
from pitivi.utils.misc import PathWalker, quote_uri, path_from_uri, generate_image_sequence_filenames
...
from pitivi.utils.imagesequences import ImageSequencePlaylist, IMAGE_SEQUENCE_PROTOCOL
from pitivi.imagesequenceeditor import ImageSequenceEditor
...
class MediaLibraryWidget(Gtk.VBox, Loggable):
    ...
    def _generateThumbnails(self, uri):
        if urlparse(uri)[0] == IMAGE_SEQUENCE_PROTOCOL:
            playlist = ImageSequencePlaylist(uri)
            playlist.parse()
            # Get the first image of the sequence to set it as the thumbnail
            middle = len(playlist.filenames) // 2
            uri = quote_uri(playlist.filenames[middle])
    ...
    def _dialogBoxResponseCb(self, dialogbox, response):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if urlparse(previewer.current_selected_uri)[0] != IMAGE_SEQUENCE_PROTOCOL:
